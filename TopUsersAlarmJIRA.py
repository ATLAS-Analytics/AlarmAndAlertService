...
df_w['cpuconsumptiontime.value'] = df_w['cpuconsumptiontime.value'].apply(
    lambda x: timedelta(seconds=int(x)).days/365.2)
...
display(df_w)
df_w.columns = ['avg attempt', 'cpusonsumption [years]',
                'failed jobs', 'jeditaskid', 'User']
print(df_w.to_string())
...
if df_w.shape[0] > 0:
    ...
    for u in S.get_immediate_subscribers(test_name):
        ...
        A.addAlert(test_name, u.name, str(
            df_w.shape[0])+' large number of failures.')
else:
    print('No Alarm')
